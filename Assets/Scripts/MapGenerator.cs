...
public class MapGenerator : MonoBehaviour {
    ...
    MapData GenerateMapData(Vector2 centre)
    {
        ...
        if (terrainData.useFalloff)
        {
            ...
            for (int y = 0; y < mapChunkSize+2; y++)
            {
                ...
                {
                    if (terrainData.useFalloff)
                        noiseMap[x, y] = Mathf.Clamp(noiseMap[x, y] - falloffMap[x, y],0f,2f);
                }
            }
        }

        textureData.ApplyToMaterial(terrainMaterial);//originale textureData.UpdateMeshHeights(terrainMaterial, terrainData.minheight, terrainData.maxheight);
    ...
    void OnValidate()
    {
        if (terrainData != null)
        {
            terrainData.OnValueUpdated -= OnValuesUpdate;
            terrainData.OnValueUpdated += OnValuesUpdate;
        }
        if (noiseData != null)
        {
            noiseData.OnValueUpdated -= OnValuesUpdate;
            noiseData.OnValueUpdated += OnValuesUpdate;
        }
        if (textureData != null)
        {
            textureData.OnValueUpdated -= OnValuesUpdate;
            textureData.OnValueUpdated += OnValuesUpdate;
        }
    }
